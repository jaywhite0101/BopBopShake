...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(switchPin), switchFlagISR, CHANGE); //flag thrown when change to voltage
  randomSeed(analogRead(0));
  
  // initial switch state read
  switchFlag = digitalRead(switchPin) == HIGH; 
  Serial.print("Initial switch state: ");
  Serial.println(switchFlag ? "ON" : "OFF");
  if (switchFlag) {
...
void switchFlagISR() {
  ...
  if (currentTime - lastDebounceTime > debounceDelay) {
    ...
    Serial.println(switchState ? "ON" : "OFF"); //debugging for if switch is on or not
    
    if (switchState != switchFlag) { 
      switchFlag = switchState;
      
      if (!switchFlag) { //checking if switch is off or if it's on (meaning to start game)
        resetGame();
        Serial.println("Switch is off");
      } else {
        Serial.println("Game started");
...
void randomColor() {
  ...
  activeColor = random(0, 2); // removed shake action, only two colors now: 0 (green) and 1 (red)
  Serial.print("Random Color - Pixel: ");
  Serial.print(activePixel);
  Serial.print(", Color: ");
  Serial.println(activeColor == 0 ? "Green" : "Red"); //debugging
  switch (activeColor) {
    case 0:
...
void youWin() {
  for (int i = 0; i < 3; ++i) {
    CircuitPlayground.playTone(723, 100); // making it higher pitched to differentiate from level up
    delay(100);
